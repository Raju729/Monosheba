        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-8">
                        ...
                        <div class="single-blog">
                            ...
                            <div class="blog-btm-meta">
                                <ul class="blg-likes">
                                    ...
                                    {{--<li><a href="{{url('admin/question-view/'.$ans->id)}}"><i class="icofont icofont-speech-comments"></i> <span>Comment</span></a></li>--}}
                                </ul>
                            </div>
                        </div>
                        <!-- comments area end -->
                        ...
                        <h4>Visitor Comments <span style="color: #0000F0">({{count($comment)}})</span></h4>
                        </div>
                        ...
                        <div class="meta-data">
                        <h2>User:{{$comments->name}}</h2>
                        {{--<p class="category">Category : <span>Health</span></p>--}}
                            @if($comments->created_at === null)
                                <p></p>
                                @else
                            <p class="category">Date of comment: <span>{{date("d-M-Y", strtotime($comments->created_at))}}</span></p>
                                @endif
                        </div>
                        ...
                        <div class="cmnt-box">
                        <form action="{{url('/user-comment')}}" method="post" id="notify">
                            {{ csrf_field() }}
                            <input type="text" name="name"  placeholder="your name" required>
                            <input type="hidden" name="question_id"  placeholder="your name" value="{{$ans->id}}">
                        <textarea name="comment"  placeholder="Message" required></textarea>
                        <input value="Send Message"  type="submit" class="btn btn-info">
                        </form>
                        </div>
                        </div>
...
@section('page-js-script')
    <script>
        $("#notify").submit(function () {
            $.notify("মন্তব্য করার জন্য ধন্যবাদ","success");
        });
    </script>
@endsection